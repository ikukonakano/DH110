...
|----------|---------------|--------|
|Good: it highlights the channel you're in so users know status/location|N/A|N/A|
|Good: in text fields, it gives a light-grey message of where you're messaging (i.e. thread of in channel), telling users where they are relative to platform|N/A|N/A|
|Good: Hovering over the button yields very clear inverted colors so users are aware it's a button|N/A|N/A|
|Good: Bolds the channels that include messages users haven't read so users are aware of changes to system|N/A|N/A|
|Good: says reconnecting when waiting for wifi to return to users are aware of wifi / updating status|N/A|N/A|
...
|Good: Can use emojis similar to texting for variable formality of messages|N/A|N/A|
|Certain keyboard commands are different when in Slack (e.g. hyperlinking on most of platforms is command+K where as in Slack it's command+shift+U|1|This could be the result of different programming standards specific to the platform. I found the solutions to keyboard commands by googling, but it would help to make it the same as the users keyboard for all other purposes|

3. user control and freedom

|Evaluation|Severity Rating|Solution|
|----------|---------------|--------|
|Good: clear X's on extra windows that users can close without closing the app, allowing for mobility around platform|N/A|N/A|
|Good: can open multiple windows within the desktop app so users can see multiple messages at once to allow for diverse use/views|N/A|N/A|
|Good: Can privately message individuals in workspace about opportunities they send in public channels allowing for user control in platform use|N/A|N/A|

4. consistency and standards

|Evaluation|Severity Rating|Solution|
|----------|---------------|--------|
|Good: jargon such as channel, thread, and workspace is used on app and common in communication platforms|N/A|N/A|
|Good: clear distinctions between the different channels are their use, making use of platform clear for users|N/A|N/A|
...
|Evaluation|Severity Rating|Solution|
|----------|---------------|--------|
|Good: Notifications are customizable for flexible options|N/A|N/A|
|Good: Can schedule slack messages so users can send at specified time|N/A|N/A|
|Good: include customizable emojis per workspace to create local language amongst users|N/A|N/A|

8. aesthetic and minimalist design

|Evaluation|Severity Rating|Solution|
|----------|---------------|--------|
|The workspace is overwhelming because there is a lot of information on one tiny screen|2|Source is owner's choice to create too many channels; reduce clutter by having less channels|
|Not prompted to look at workspace unless it is already open or you have deliberate action to take there|2|Source is minimizing superfluous notifications at the cost of seeing message; send reminder notifications about unread messages in recently unvisited channels|
...
|Evaluation|Severity Rating|Solution|
|----------|---------------|--------|
|Good: They have standard page titles such as "Home", "About Us", etc. which many websites have|N/A|N/A|
|Good: Have whole pages dedicated to organization specific terms such as collective impact and Good Food For All Agenda which creates local language between users|N/A|N/A|
...
|Good: When donating, you're given suggestions for values donation that you can click, and options of repeating or one-time donations are in a drop-down to promote certain donation value norms|N/A|N/A|
...
|Users do not know they can get recurring notifications via email about LAFPC for updates and information|1|Source is a lack of user reminders; have mailing list sign-up be the first prompt users get|
...
|Evaluation|Severity Rating|Solution|
|----------|---------------|--------|
|Good: Consistent colors throughout the website that match their logo for aesthetics|N/A|N/A|
|Could have the mailing list email field be in line with aesthetic of rest of website|1|source is preset standards for their field; change style to match|
...
|Evaluation|Severity Rating|Solution|
|----------|---------------|--------|
|Good: Thorough explanations of their local terms for user understanding|N/A|N/A|
|Unclear what being a member of a working group involved|2|Source is lack of documentation; create a small section of information on top of working group breakdown to explain what it means to be a member|
